...
<template>
        <v-main id="books">
         <v-img class="book" v-bind:height="changeSize(item)['height']" v-bind:width="changeSize(item)['width']" v-for="(item, key) in categoryItems" :key="key" :src="item.img_path" @click="openDetail(item)" 
            alt=""></v-img>
...
export default {
    ...
    methods: {
      changeSize(item) {
        let size_dict = {
          "s": {"height": 200, "width": 150}, 
          "m": {"height": 300, "width": 200}, 
          "l": {"height": 500, "width": 300},
          }
        return size_dict[item.size];
      },
      openDetail(item) {
